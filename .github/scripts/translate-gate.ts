...
async function api<T>(path: string): Promise<T> {
	...
	if (!response.ok) {
		throw new Error(
			`GitHub API request failed: ${path}: ${response.status} ${response.statusText}`,
		);
	}
...
async function main(): Promise<void> {
	...
	if (!allowedPermissions.has(permission)) {
		throw new Error(
			`User ${commentAuthor} has insufficient permission: ${permission}`,
		);
	}
	...
	if (pullRequest.state !== "open") {
		throw new Error(
			`Pull request #${pullRequestNumber} is not open: ${pullRequest.state}`,
		);
	}
	...
	if (!changedFiles.has(requiredChangedFile)) {
		throw new Error(
			`Pull request #${pullRequestNumber} does not change ${requiredChangedFile}`,
		);
	}
