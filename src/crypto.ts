...
function decrypt(passphrase: string, encData: string): Promise<string> {
  ...
  return deriveKey(passphrase, data.slice(8, 16))
    .then(({ iv, key }) =>
      window.crypto.subtle.decrypt({ iv, name: "AES-CBC" }, key, data.slice(16)),
    )
...
function deriveKey(passphrase: string, salt: Uint8Array): any {
  return window.crypto.subtle
    ...
    .then((passwordKey) =>
      window.crypto.subtle.deriveBits({ ...pbkdf2Params, salt }, passwordKey, 384),
    )
    .then((key) =>
...
function encrypt(passphrase: string, salt: Uint8Array, plainData: string): Promise<string> {
  return deriveKey(passphrase, salt)
    ...
    .then((encData) => {
      const banner = opensslBanner;
      const cipherBytes = new Uint8Array(encData);
      const data = new Uint8Array(banner.byteLength + salt.byteLength + cipherBytes.byteLength);
      data.set(banner, 0);
      data.set(salt, banner.byteLength);
      data.set(cipherBytes, banner.byteLength + salt.byteLength);
      return data;
    })
